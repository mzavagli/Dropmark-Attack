...
static void 
address_to_subip(char *address, int *subip) 
{
...
static void 
subip_to_subip_bin(uint8_t subip, char *subip_bin) 
{
  int k;
  for (int i=7; i>=0; i--) {
    k = subip >> i;
...
void 
signal_send_delayed_destroy_cb(void *arg) 
{
  circuit_t *circ = arg;
...
static void 
signal_send_one_cell_cb(void *arg)
{
  dropmark_encode_state_t *state = arg;
  if (!state->circ) {
    log_info(LD_GENERAL, "DROPMARK: Circuit has been freed before the callback. Signal not sent");
    return;
  }
  if (state->circ->marked_for_close) {
    log_info(LD_GENERAL, "DROPMARK: Circuit has been marked for close. Signal not sent");
    return;
  }
  if (signal_send_relay_drop(1, state->circ) < 0) {
    log_info(LD_GENERAL, "DROPMARK: BUG, final cell not send");
  }
...
static void 
signal_bandwidth_efficient_cb(evutil_socket_t fd, short events, void *arg) 
{
  ...
  log_info(LD_GENERAL, "DROPMARK: Callback called at time %u:%ld", (int)now.tv_sec, now.tv_nsec);
  if (subip_bin[idx] == '0') {
    if (signal_send_relay_drop(2, state->circ) < 0) {
      log_info(LD_GENERAL, "DROPMARK: BUG: signal_send_relay_drop returned -1 on a 0 bit sending");
    }
  }
  else if (subip_bin[idx] == '1') {
    if (signal_send_relay_drop(3, state->circ) < 0) {
      log_info(LD_GENERAL, "DROPMARK: BUG: signal_send_relay_drop_returned -1 on 1 bit sending");
    }
  }
  else {
    log_info(LD_GENERAL, "DROPMARK: BUG: something went wrong with subip_bin: %s", subip_bin);
  }
  if (!CIRCUIT_IS_ORIGIN(state->circ)) {
    channel_flush_some_cells(TO_OR_CIRCUIT(state->circ)->p_chan, -1);
    connection_flush(TO_CONN(BASE_CHAN_TO_TLS(TO_OR_CIRCUIT(state->circ)->p_chan)->conn));
...
static int 
signal_bandwidth_efficient(char *address, circuit_t *circ) 
{
  dropmark_encode_state_t *state = tor_malloc_zero(sizeof(dropmark_encode_state_t));
  state->circ = circ;
  int subip[4];
...
static void 
signal_minimize_blank_latency_cb(void *arg) 
{
  dropmark_encode_state_t *state = arg;
  struct timeval timeout =  {0, get_options()->SignalBlankIntervalMS*1E3};
  ...
  if (!CIRCUIT_IS_ORIGIN(state->circ)) {
    ...
    log_info(LD_GENERAL, "DROPMARK: connection_flush called and returned %d", r);
  }
  if (state->nb_calls < 3) {
    state->nb_calls++;
    evtimer_add(state->ev, &timeout);
...
static int 
signal_minimize_blank_latency(char *address, circuit_t *circ) 
{
...
/*
struct timespec* list_now[4];
struct smartlist_t* list_circ;
int time_list_index = 0;
double dropmark_activation_threshhold = 1000;
double dropmark_following_threshhold = 1;

int signal_listen_and_decode_m(circuit_t *circ) {
  struct timespec *now = tor_malloc_zero(sizeof(struct timespec));
  clock_gettime(CLOCK_REALTIME, now);
  list_now[time_list_index] = now;
  time_list_index++;
  if (time_list_index == 4) {
    double elapsed_ms_0_1 = list_now[1]->tv_sec*1000+list_now[1]->tv_nsec*1E-6 - list_now[0]->tv_sec*1000+list_now[0]->tv_nsec*1E-6;
    // log_info(LD_GENERAL, "DROPMARK: now: %f, %f", list_now[1]->tv_sec*1000+list_now[1]->tv_nsec*1E-6, list_now[0]->tv_sec*1000+list_now[0]->tv_nsec*1E-6);
    double elapsed_ms_1_2 = list_now[2]->tv_sec*1000+list_now[2]->tv_nsec*1E-6 - list_now[1]->tv_sec*1000+list_now[1]->tv_nsec*1E-6;
    double elapsed_ms_2_3 = list_now[3]->tv_sec*1000+list_now[3]->tv_nsec*1E-6 - list_now[2]->tv_sec*1000+list_now[2]->tv_nsec*1E-6;
    if ((elapsed_ms_0_1 > dropmark_activation_threshhold) && (elapsed_ms_1_2 < dropmark_following_threshhold) && (elapsed_ms_2_3 < dropmark_following_threshhold)) {
      log_info(LD_GENERAL, "DROPMARK: SPOTTED PTN, %f, %f, %f", elapsed_ms_0_1, elapsed_ms_1_2, elapsed_ms_2_3);
    }
    list_now[0] = list_now[1];
    list_now[1] = list_now[2];
    list_now[2] = list_now[3];
    list_now[3] = NULL;
    time_list_index--;
  }
  tor_free(now);
...
STATIC int signal_minimize_blank_latency_decode(dropmark_decode_t *circ_timing) {
    ...
    switch (delta_timing(smartlist_get(circ_timing->timespec_list, i-count),
      ...
      case 0:
        ...
        if (ipcount == 3) {
          ...
          log_info(LD_GENERAL, "DROPMARK: Dest IP : %d.%d.%d.%d",
      ...
      case 1:
        ...
        if (count > 258) {
          ...
          log_info(LD_GENERAL, "DROPMARK: count value above the limit, removing packets");
          return 0;
        }
        break;
      case 2:
...
signal_decode_simple_watermark(dropmark_decode_t *circ_timing, char *p_addr, char *n_addr) {
    if (smartlist_len(circ_timing->timespec_list) == 4) {
        ...
        if (delta_timing(smartlist_get(circ_timing->timespec_list, 0), smartlist_get(circ_timing->timespec_list, 1)) == 1) {count++;}
        ...
        if (delta_timing(smartlist_get(circ_timing->timespec_list, 0), smartlist_get(circ_timing->timespec_list, 2)) == 1) {count++;}
        if (delta_timing(smartlist_get(circ_timing->timespec_list, 1), smartlist_get(circ_timing->timespec_list, 3)) == 1) {count++;}
        
        if (count >= 3) {log_info(LD_GENERAL, "DROPMARK: Spotted watermark, predecessor: %s, successor: %s", p_addr, n_addr);}
        ...
        if (smartlist_len(circ_timing->timespec_list) == 10) {
            circ_timing->disabled = 1;
            log_info(LD_GENERAL, "DROPMARK: timespec_list 10 disabled");
        }
        return 0;
    }
}
...
static int 
signal_bandwidth_efficient_decode(dropmark_decode_t *circ_timing, char *p_addr, char *n_addr) 
{
  /*
      ...
      case 0:
        if (count == 2) 
          ...
          if (nbr_sub_ip_decoded > 0) {
            circ_timing->disabled = 1;
            log_info(LD_GENERAL, "DROPMARK: Signal distorded or no signal, count: %d, predecessor: %s, sucessor: %s",
                count, p_addr, n_addr);
            return 0;
          }
          count = 1;
        ...
        if (nth_bit > 7) {
          ...
          if (nbr_sub_ip_decoded == 3) {
            log_info(LD_GENERAL, "DROPMARK: Dest IP in binary: %s.%s.%s.%s, predecessor: %s, successor: %s",
      ...
      case 2:
        if (nbr_sub_ip_decoded == 3 && nth_bit == 7) {
          if (count == 2)
            ...
            log_info(LD_GENERAL, "DROPMARK: signal distorded: %s.%s.%s.%s - count %d, predecessor: %s, successor: %s",
...
void
signal_encode_state_free(dropmark_encode_state_t *state) 
{
